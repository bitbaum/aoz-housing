...
async function main() {
  ...
  privateRooms.forEach(p => console.log(`      ${p.code} - ${p.label} (requires medical: ${p.requiresMedicalDocs})`))
  ...
  console.log(`   Carlos: http://localhost:3000/residents/${residents.find(r => r.code === 'WIT-005')?.id}`)
