...
if [ -z "$base_ci" ]; then
  ...
  # A red base must not become a trap for the fix.
  #
  # "Never merge onto red" is right for an unrelated change: it stops a broken
  # base collecting more changes and getting harder to diagnose. But when the
  # PR *is* the repair, that same rule deadlocks the repo — the fix cannot
  # travel the path its own redness blocks, and nothing but a human can move
  # it. Observed in maonakamoto/aoz-housing on 2026-08-07: E2E red on master,
  # the fix sitting green in a PR, and every sweep politely refusing. That is
  # the same shape as the cancelled-base deadlock this script already learned.
  #
  # So: identify WHICH jobs are failing, and let a PR through only if its own
  # checks pass every one of them. That is not a weakening — a PR's checks run
  # on the MERGE result (refs/pull/N/merge), so green-on-those-jobs is direct
  # evidence that the post-merge base is better than the pre-merge base. A PR
  # that does not cover the failing jobs is still refused.
  base_red_jobs=""
