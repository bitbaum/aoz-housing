...
test.describe('Analytics page (/analytics)', () => {
  test('loads with heading and subtitle', async ({ page }) => {
    ...
    // PAGE_TITLES.analytics = 'Auswertung'
  ...
  test('shows Mission KPI section', async ({ page }) => {
    ...
    // MISSION_KPI_LABELS.sectionTitle = 'Missions-KPIs'
    await expect(
  ...
  test('shows recent placements table', async ({ page }) => {
    ...
    // RecentPlacementsTable renders "Neueste Platzierungen (90 Tage)"
    await expect(
  ...
  test('accessible from header navigation', async ({ page }) => {
    ...
    // Analytics lives in the "Monitoring" megamenu dropdown (opens on hover)
    await page.locator('header').getByRole('button', { name: 'Monitoring' }).hover()
    const navLink = page.locator('header a[href="/analytics"]').first()
...
test.describe('Admin chores (/chores)', () => {
  test('loads with heading', async ({ page }) => {
    ...
    // CHORE_LABELS.admin.pageTitle = 'Haushaltsaufgaben'
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 15_000 })
    await expect(page.getByRole('heading', { name: /Haushaltsaufgaben|Aufgaben/i }).first()).toBeVisible()
  })
  ...
  test('shows per-unit summary with seed units', async ({ page }) => {
    ...
    // The page renders mobile cards (hidden on desktop) and a desktop table,
    // so scope to the visible desktop table.
    await expect(
      page.locator('table').locator('text=/ZH-\\d{3}/').first()
    ).toBeVisible({ timeout: 15_000 })
  })
  ...
  test('unit codes link to housing detail page', async ({ page }) => {
    ...
    // Desktop table variant (mobile cards are display:none at this viewport)
    const unitLink = page.locator('table a[href^="/housing/"]').first()
  ...
  test('accessible from header navigation', async ({ page }) => {
    ...
    // Chores lives in the "Unterkünfte" megamenu dropdown (opens on hover)
    await page.locator('header').getByRole('button', { name: 'Unterkünfte' }).hover()
    const navLink = page.locator('header a[href="/chores"]').first()
    await expect(navLink).toBeVisible({ timeout: 15_000 })
