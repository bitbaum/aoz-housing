...
test.describe('Navigation — admin pages load', () => {
  ...
  test('housing list loads', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Unterkünfte/i })).toBeVisible({
      timeout: 30_000,
    })
  })
  ...
  test('incidents list loads', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Vorfälle/i })).toBeVisible({
      timeout: 30_000,
    })
  })
  ...
  test('placements list loads', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Platzierung/i })).toBeVisible({
      timeout: 30_000,
    })
  })
  ...
  test('matching page loads', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Matching/i })).toBeVisible({
      timeout: 30_000,
    })
  })
