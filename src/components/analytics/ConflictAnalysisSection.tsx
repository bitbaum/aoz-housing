    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mt-4 sm:mt-6">
      ...
      <div className="card border-l-4 border-status-warning">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-status-warning-text text-xl" aria-hidden="true">
            📊
          </span>
        ...
        {Object.keys(conflictsByGap).length === 0 ? (
          <div className="text-center py-6">
            <p className="text-ui-muted text-sm">{ALGORITHM_ACCURACY_LABELS.conflictCausesEmpty}</p>
      ...
      <div className="card border-l-4 border-status-info">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-status-info-text text-xl" aria-hidden="true">
            🔮
          </span>
          <h2 className="text-lg font-semibold text-ui-text">
        ...
        {conflictPlacementsCount === 0 ? (
          <div className="text-center py-6">
            <p className="text-ui-muted text-sm">{ALGORITHM_ACCURACY_LABELS.predictabilityEmpty}</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-3 bg-status-success/10 rounded-lg text-center">
                <p className="text-2xl font-bold text-status-success">{predictableCount}</p>
                <p className="text-xs text-status-success-text">
                  {ALGORITHM_ACCURACY_LABELS.predictableLabel}
                </p>
              </div>
              <div className="p-3 bg-status-error/8 rounded-lg text-center">
                <p className="text-2xl font-bold text-status-error">{unpredictableCount}</p>
                <p className="text-xs text-status-error-text">
                  {ALGORITHM_ACCURACY_LABELS.unpredictableLabel}
                </p>
              </div>
            </div>
            ...
            {totalPredictability > 0 && (
              <div className="p-3 bg-ui-subtle rounded-lg">
                <p className="text-sm text-ui-muted">
                  <strong>{Math.round((predictableCount / totalPredictability) * 100)}%</strong>{' '}
