/**
 * Resident components barrel export
 */

export * from './PlacementActions'
export * from './TransferRecommendations'
export * from './TransferUnitSelector'
export * from './SatisfactionHistory'
export * from './ResidentProfileSidebar'
export * from './ResidentIncidents'
export * from './CompatibleMatchesCard'
export * from './TopCompatibilitiesCard'
export * from './PlacementHistoryCard'
export * from './LearningForm'
export * from './LearningRecordsCard'
export * from './CareTeamCard'
export * from './CareWorkspace'
export * from './ResidentDocumentsCard'
export * from './ClientBoard'
