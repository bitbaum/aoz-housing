...
const RESIDENT_BLOCKER_LABELS: Record<string, string> = {
  ...
  incidentsAsSubject: 'Vorfälle als betroffene Person',
  involvements: 'Vorfallsbeteiligungen',
  maintenanceRequests: 'Wartungsmeldungen',
  assessments: 'Kompatibilitäts-Assessments',
}
...
export function ResidentDangerZone({ residentId, residentCode }: Props) {
  ...
  return (
    <section className="mt-8 p-4 sm:p-5 border-2 border-red-300 rounded-lg bg-red-50" aria-labelledby="resident-danger-zone-title">
      <h2 id="resident-danger-zone-title" className="text-base sm:text-lg font-semibold text-red-900">Danger Zone — Hard-Delete</h2>
      <p className="text-sm text-red-800 mt-1 leading-relaxed">
        Nur für Test-/Demo-Bewohner. Diese Aktion ist endgültig und entfernt den Datensatz.
      </p>

      {!isEligibleCode && (
        <p className="mt-3 text-sm text-red-700">
          Dieser Bewohner-Code ist nicht als Test/Demo markiert. Hard-Delete ist gesperrt.
        </p>
      )}
      ...
      <button
        ...
        onClick={() => {
          ...
          startTransition(async () => {
            ...
            setFeedback({ kind: 'success', text: 'Bewohner wurde endgültig gelöscht' })
