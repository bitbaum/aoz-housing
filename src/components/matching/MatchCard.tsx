...
export function MatchCard({ match, resident, rank }: Props) {
  ...
  const roommateLanguages = match.unit.placements.flatMap((p) => p.resident.languages || [])
  ...
  return (
    <div
      className={`p-4 border rounded-lg ${hasBlockingIssues ? 'border-status-error/25 bg-status-error/8' : rank === 1 ? 'border-score-excellent/30 bg-score-excellent/8' : rank && rank <= 3 ? 'border-status-info/20 bg-status-info/8' : 'border-ui-border'}`}
    >
      <div className="flex items-start justify-between mb-3 gap-2">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            {rank && rank <= 3 && (
              <span
                className={`chip font-semibold ${rank === 1 ? SCORE_TOKENS.excellent.soft : SCORE_TOKENS.good.soft}`}
              >
        ...
        <div className="text-right">
          ...
          {match.bestRoomFit && match.bestRoomFit.score === null && occupancy > 0 && (
            <p className="text-xs text-ui-muted mt-1">
              Freies Zimmer — keine Mitbewohnenden im Zimmer
            </p>
          )}
        </div>
      </div>
      ...
      {match.unitMetrics && (
        <div
          className={`mb-3 p-2 rounded border ${
            match.unitMetrics.riskLevel === 'CRITICAL'
              ? 'bg-severity-critical/10 border-severity-critical/30'
              : match.unitMetrics.riskLevel === 'HIGH'
                ? 'bg-severity-high/10 border-severity-high/30'
                : match.unitMetrics.riskLevel === 'MEDIUM'
            ...
            <>
              <div className="flex items-center justify-between">
                <span className="text-xs text-brand-secondary">
                  📊 {MATCHING_LABELS.historicalData} ({realSuccessData.totalPlacements}{' '}
                  {MATCHING_LABELS.placements}):
                </span>
                <span
                  className={`text-xs font-bold ${getScoreColorClass(realSuccessData.successRate)}`}
                >
                  {realSuccessData.successRate}% {MATCHING_LABELS.successRate}
                </span>
              </div>
              <p className="text-xs text-brand-secondary/80 mt-1">
                {realSuccessData.successfulPlacements}/{realSuccessData.totalPlacements}{' '}
                {MATCHING_LABELS.placementsWithSimilar} ({match.apartmentFit.fitScore}%{' '}
                {MATCHING_LABELS.plusMinusTen}){' '}
                {MATCHING_LABELS.successRateDesc(realSuccessData.successRate)}
              </p>
            </>
          ) : (
            <div className="text-xs text-brand-secondary/80">
              📊 {MATCHING_LABELS.noHistoricalData} ({match.apartmentFit.fitScore}%{' '}
              {MATCHING_LABELS.plusMinusTen}) {MATCHING_LABELS.available}.
            </div>
          )}
        </div>
      )}
        ...
        <div className="mb-3 space-y-1">
          {occupancy === 0 && (
            <p className="text-xs text-status-success-text">
              ✓ {MATCHING_LABELS.noRoommatesNoConflicts}
            </p>
          )}
          {sharedLanguages.length > 0 && (
            <p className="text-xs text-status-success-text">
              ✓ {MATCHING_LABELS.sharedLanguageWith}{' '}
              {
                match.unit.placements.filter((p) =>
                  (resident.languages || []).some((l: string) =>
                    (p.resident.languages || []).includes(l),
                  ),
                ).length
              }{' '}
              {MATCHING_LABELS.residentsCount}
            </p>
          )}
        ...
        <div className="mb-3">
          ...
          <div className="space-y-1">
            {match.unit.placements.map((p) => {
              ...
              const hasSharedLang = (resident.languages || []).some((l: string) =>
                (p.resident.languages || []).includes(l),
              )
              ...
              return (
                <div key={p.id} className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={
                      concernCount > 0 ? 'text-status-warning-text' : 'text-status-success-text'
                    }
                  >
        ...
        <div className="mb-3">
          ...
          {allConcerns.length > DISPLAY_LIMITS.matchConcerns && (
            <p className="text-xs text-ui-muted">
              {MATCHING_LABELS.moreConcerns(allConcerns.length - DISPLAY_LIMITS.matchConcerns)}
            </p>
          )}
        </div>
      )}
      ...
      {match.unit.spots && match.unit.spots.length > 0 && (
        <SpotSelection spots={match.unit.spots} resident={resident} match={match} />
      )}

      {(!match.unit.spots || match.unit.spots.length === 0) &&
        (() => {
          const hasBlockingConflicts =
            match.apartmentFit?.conflicts.some(
