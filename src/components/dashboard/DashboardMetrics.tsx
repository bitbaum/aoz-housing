  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ...
      <Link href="/residents" className="card-hover">
        ...
        <p
          className={`text-sm mt-2 ${unplacedCount > 0 ? 'text-status-warning' : 'text-ui-muted'}`}
        >
          {unplacedCount > 0
            ? ALGORITHM_ACCURACY_LABELS.statWaitingForPlacement(unplacedCount)
            : ALGORITHM_ACCURACY_LABELS.statAllPlaced}
        </p>
      </Link>
      ...
      <Link href="/housing" className="card-hover">
        ...
        <p
          className={`text-3xl font-bold mt-1 ${freeBeds >= 5 ? 'text-status-success' : freeBeds >= 2 ? 'text-status-warning' : 'text-status-error'}`}
        >
          {freeBeds}
        </p>
      ...
      <Link href="/placements?status=active&overdue=1" className="card-hover">
        ...
        <p
          className={`text-3xl font-bold mt-1 ${overdueCheckIns === 0 ? 'text-status-success' : overdueCheckIns <= 3 ? 'text-status-warning' : 'text-status-error'}`}
        >
          {overdueCheckIns}
        </p>
        <p className="text-sm mt-2 text-ui-muted">
      ...
      <Link href="/incidents" className="card-hover">
        ...
        <p
          className={`text-3xl font-bold mt-1 ${openIncidents === 0 ? 'text-status-success' : openIncidents <= 5 ? 'text-status-warning' : 'text-status-error'}`}
        >
          {openIncidents}
        </p>
        <p className="text-sm mt-2 text-ui-muted">
