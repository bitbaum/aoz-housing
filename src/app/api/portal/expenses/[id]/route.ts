...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const expense = await prisma.expense.findUnique({
      where: { id: params.id },
      select: { id: true, housingUnitId: true, paidById: true, createdById: true, amountRappen: true },
    })
    ...
    if (!expense || expense.housingUnitId !== auth.placement.housingUnitId) {
      return NextResponse.json({ success: false, error: ERROR_MESSAGES.EXPENSE_NOT_FOUND }, { status: 404 })
    }
    ...
    return NextResponse.json({ success: false, error: ERROR_MESSAGES.EXPENSE_DELETE_ERROR }, { status: 500 })
  }
}
