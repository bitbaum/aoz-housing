...
import { getSeverityBorderClass, formatRelativeDate, formatDate } from '@/lib/utils'
...
export default async function IncidentDetailPage({ params, searchParams }: Props) {
    ...
    new Map(
      [
        ...
        ...(incident.subjectId && incident.subject
          ...
          : []),
        ...incident.involvedResidents.map((i) => [i.residentId, residentName(i.resident)] as const),
  ...
  const isOverdue =
    incident.nextFollowUpDate && incident.nextFollowUpDate < new Date() && !incident.resolvedAt
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mt-2">
          <div className="flex items-center gap-4">
            <span
              className="text-3xl"
              role="img"
              aria-label={getLabel(INCIDENT_CATEGORY_LABELS, incident.category)}
            >
      ...
      {isOverdue && (
        <div className="mb-6 p-4 bg-status-error/8 border border-status-error/25 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-2xl" role="img" aria-label={INCIDENT_DETAIL_LABELS.overdueAria}>
                ⏰
              </span>
              <div>
                <p className="font-semibold text-status-error-text">
                  {INCIDENT_DETAIL_LABELS.overdueTitle(
                    formatRelativeDate(incident.nextFollowUpDate!),
                  )}
                </p>
                {incident.followUpPriority && (
                  <p className="text-sm text-status-error-text">
                    {INCIDENT_DETAIL_LABELS.priorityPrefix}
      ...
      {incident.nextFollowUpDate && !isOverdue && !incident.resolvedAt && (
        <div className="mb-6 p-4 bg-status-info/8 border border-status-info/25 rounded-lg">
          <div className="flex items-center gap-3">
            <span className="text-2xl" role="img" aria-label={INCIDENT_DETAIL_LABELS.scheduledAria}>
              📅
            </span>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`card border-l-4 ${getSeverityBorderClass(incident.severity)}`}>
            ...
            <p className="text-ui-muted whitespace-pre-wrap">{incident.description}</p>

            {incident.resolution && (
              <div className="mt-4 pt-4 border-t border-ui-border">
                <h3 className="font-medium text-ui-text mb-2">
                  {INCIDENT_DETAIL_LABELS.resolutionTitle}
                </h3>
